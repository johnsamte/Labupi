...
import com.johnsamte.labupi.databinding.ActivityContentBnlBinding

class ContentBnlActivity : AppCompatActivity() {

    private lateinit var binding: ActivityContentBnlBinding
    private lateinit var adapter: ContentBnlAdapter
    private var isSortedByTitle = false

    override fun onCreate(savedInstanceState: Bundle?) {

        super.onCreate(savedInstanceState)
        binding = ActivityContentBnlBinding.inflate(layoutInflater)
        setContentView(binding.root)

        setupToolbar()

        val dbHelper = LabuDatabaseHelper(this)
        val bnlContent = dbHelper.getBNLContent()

        adapter = ContentBnlAdapter { clickedItem ->
            // Handle click and navigate to the new activity
            val intent = Intent(this, BnlActivity::class.java).apply {
        ...
        binding.biaknalaRecycler.layoutManager = LinearLayoutManager(this)
        binding.biaknalaRecycler.adapter = adapter

        // 🔑 Feed data into adapter
        adapter.submitFullList(bnlContent)
    }
